...
iDistrict_tibble <- fread("dashboard/iDistrict_table.csv") %>% 
  dplyr::select(-osm_id)
...
leaflet(imap) %>%
  ...
  # leaflet::addLegend("bottomright",pal = pal_line, values = ~col_scale, title = "tCO2/m2/month",
  #                    labFormat = labelFormat(prefix = "  > " , transform =  function(x) exp(x) - 1 , digits = 0)) %>% 
  leaflet::addLegend("bottomright", pal = pal_line, values = "", title = "tCO2/m2/month") %>%
  # layers control 
  addLayersControl(
