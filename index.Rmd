...
imap <- st_read("dashboard/imap.shp")
trend <- fread("dashboard/trend.csv")
mapCount <- fread("dashboard/mapCount.csv")
amenity <- fread("dashboard/amenity.csv")
natural <- fread("dashboard/natural.csv")
leisure <- fread("dashboard/leisure.csv")
shop <- fread("dashboard/shop.csv")
place <- fread("dashboard/place.csv")
OSM_road <- fread("dashboard/OSM_raod.csv")
landuse <- fread("dashboard/landuse.csv")
street <- fread("dashboard/street.csv")
hour <- fread("dashboard/hour.csv")
find_street <- fread("dashboard/find_street.csv")
week <- fread("dashboard/week.csv")
iDistrict_map <- st_read("dashboard/iDistrict.shp")
```
...
# iDistrict {data-icon="fa-map"}

### Emission totals per district {data-height=600}
...
leaflet(iDistrict_map) %>%
  ...
  # layers control
  addLayersControl(
    baseGroups = c("CartoDB.DarkMatter", "OpenStreeMap.Default", "Esri.WorldImagery"),
    position = "topleft",
    options = layersControlOptions(collapsed = TRUE)
  )
...
### Emitter disctrict {data-height=500}

```{r}
custom_colors <- viridis::viridis(n = length(iDistrict_map$tCO2))

# Leisure Open Street Map feature
iDistrict_map %>%
  group_by(district) %>%
  arrange(desc(tCO2)) %>% 
  hchart('column', hcaes(x = district, y = tCO2, color = custom_colors)) %>% 
  hc_add_theme(hc_theme_google()) %>% 
  hc_tooltip(pointFormat = '<b>ktCO2: </b> {point.y} <br>') %>% 
  hc_title(text = 'Traffic emissions per district',
           style = list(fontSize = '25px', fontWeight = 'bold')) %>% 
  hc_subtitle(text = 'tCO2/month',
              style = list(fontSize = '16px')) %>% 
  hc_xAxis(title = "") %>%
  hc_yAxis(title = "") %>%
  hc_credits(enabled = TRUE, text = '@ZoomCityCarbonModel')%>% 
